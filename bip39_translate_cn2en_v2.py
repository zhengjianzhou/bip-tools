...
### with open('chinese_simplified.txt', 'r') as f: cn=''.join(f.readlines()).strip().split('\n')
### with open('english.txt', 'r') as f: en=''.join(f.readlines()).strip().split('\n')

### below 2 lists are sourced from : https://github.com/bitcoin/bips/tree/master/bip-0039
### as of 8-Apr-2024

chinese_simplified_list = '''
的
一
是
在
...
class Bip39Check(object):
    def __init__(self):
        ...
        for w in english_list.strip().split('\n'):
...
cn=''.join(chinese_simplified_list).strip().split('\n')
en=''.join(english_list).strip().split('\n')

CN_IC_dict = dict([(i, cn[i]) for i in range(len(cn))])
EN_IC_dict = dict([(i, en[i]) for i in range(len(en))])
CN_CI_dict = dict([(cn[i], i) for i in range(len(cn))])
EN_CI_dict = dict([(en[i], i) for i in range(len(en))])
...
### last word of the seed phrase are generated automatically
cn_length = cn_length - 1

cn_en_dict = dict(zip(cn,en))
en_cn_dict = dict(zip(en, cn))
cn_input = input(f'Please type in your {cn_length} Chinese characters (It will only take first {cn_length} if too long. It will repeat from beginning if too short.)\n===>')

cn_delete = ''
cn_use = ''
for s in cn_input:
    if s in cn_en_dict.keys():
        cn_use += s
    else:
        cn_delete += s

cn_delete = set(cn_delete)
print(f"{cn_delete} is not in the list of the 2048 Chinese characters. They are REMOVED from the phrase generation!!!\nPlease take notes!!! Effective input is {cn_use}")

if len(cn_use) < 6:
    print("Please use a longer phrase!!! At least 6 characters!!!")
    exit()

passcode_str = input(f'Please enter a pass code:\nIt can be in any length. It can be empty. It can be any typable ascii characters (except leading/ending space). It is case sensitive!\nPlease remember this code - without this code you will not be able to retrieve the target pass phrase!!!\nE.g. 12354, or A1123xx$#@, etc.\n===>')
passcode_str = passcode_str.strip()

passcode = [0] * cn_length  ### sett all into 1, so that the number times itself and take the remainder of division over LEN_CN should be itself
if passcode_str:
    passcode = ascii_to_ord_list(passcode_str)
    passcode = (passcode * cn_length)[:cn_length]

print(f'Selected passcode: {passcode_str}\n{passcode}')

cn_output = ''
en_output = []
i = 0
while len(cn_output) < cn_length:
    for s in cn_use:
        if len(cn_output) >= cn_length: break

        ### old method without passcode
        # cn_output += s
        # en_output.append(cn_en_dict[s])

        ### new method with passcode
        cn_output += s
        cn_ord = CN_CI_dict[s]
        pcode = passcode[i] * (i+1)                           ### logic : passcode to time with ordinals - to avoid repeating words, if won't change the behavior when code is 0 - meaning no passcode provided
        en_ord = ( cn_ord * (pcode+1) + pcode) % LEN_CN   ### logic : c = (m * (p+1) + p) mod ( TOTAL_CHARs ) ---> c: coded number, m: message number, p: passcode
        en_encoded = EN_IC_dict[en_ord]
        en_output.append(en_encoded)

        i += 1

m = Bip39Check()
m._check_size(en_output[:cn_length])
m._compute_entropy(en_output[:cn_length])
last_word = m._scan()
print(f'Auto-generated checksum last_word is: {last_word}, {en_cn_dict[last_word]}')

en_output, cn_output = en_output[:cn_length] + [last_word], cn_output[:cn_length] + en_cn_dict[last_word]
